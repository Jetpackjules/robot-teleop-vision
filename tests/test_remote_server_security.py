...
def bare_handler(headers=None, server=None):
    ...
    handler.server = server or SimpleNamespace()
    return handler
...
def test_quick_tunnel_cannot_authorize_arm_but_named_access_can():
    ...
    assert not quick.arm_authorized()
    named = bare_handler(
        {
...
def test_only_one_arm_controller_can_hold_lease(tmp_path):
    ...
    try:
        first = server.claim_arm_controller("first")
        assert first
        assert server.claim_arm_controller("second") is None
        server.release_arm_controller(first)
        assert server.claim_arm_controller("second")
    finally:
        server.server_close()
...
def test_arm_transport_jitter_drops_one_packet_without_forcing_hold(tmp_path):
    ...
    try:
        ...
        accepted, age_ms = server.normalize_browser_arm_timing(initial, 10_000)
        assert accepted
        assert age_ms == 0

        delayed = {"sent_unix_ms": 9_600}
        accepted, age_ms = server.normalize_browser_arm_timing(delayed, 10_000)
        assert not accepted
        assert age_ms == 300
        assert server.arm_stale_command_drop_count == 1

        fresh = {"sent_unix_ms": 9_966}
        accepted, age_ms = server.normalize_browser_arm_timing(fresh, 10_066)
        assert accepted
        assert age_ms == 0
    finally:
        server.server_close()
...
def test_arm_transport_rebases_after_stable_latency_path_shift(tmp_path):
    ...
    try:
        assert server.normalize_browser_arm_timing(
            {"sent_unix_ms": 9_900},
            10_000,
        )[0]
        ...
        for index, offset in enumerate(offsets):
            ...
            decisions.append(
                server.normalize_browser_arm_timing(message, bridge_ms)[0]
            )
        assert decisions == [False, False, False, True]
        assert server.arm_browser_clock_offset_ms == pytest.approx(404, abs=1)
    finally:
        server.server_close()
...
def test_keyboard_auto_resume_only_allows_same_session_watchdog_pause(tmp_path):
    ...
    forwarded = []
    try:
        controller_id = server.claim_arm_controller("browser")
        assert controller_id
        with server.arm_status_lock:
            server.arm_status = {
                "type": "arm_status",
                "state": "hold",
            ...
            server.arm_status_received_at = time.monotonic()
        server.forward_arm = forwarded.append
        command = {"type": "arm_cartesian_velocity", "deadman": True}
        assert server.resume_keyboard_after_transient_watchdog(
            controller_id,
            command,
        )
        assert forwarded == [
        ...
        with server.arm_status_lock:
            server.arm_status["message"] = (
                "Contact / following error stopped motion at servo 2"
            )
        assert not server.resume_keyboard_after_transient_watchdog(
            controller_id,
            command,
        )
...
def test_newest_page_preempts_stream_and_arm_control_together(tmp_path):
    ...
    class FakeHandler:
        def __init__(self):
            self.arm_preempted = False
            self.rgbd_preempted = False

        def preempt_arm_websocket(self):
            self.arm_preempted = True
    ...
    server.forward_arm = forwarded.append
    try:
        assert server.claim_arm_controller("old", "old-page", old_arm)
        assert server.claim_rgbd_client("new-page", new_rgbd)
        assert old_arm.arm_preempted
...
def test_robot_calibration_status_is_merged_into_arm_status(tmp_path):
    server = LanRemoteServer(
        ("127.0.0.1", 0),
        LanRemoteHandler,
        tmp_path,
    ...
    try:
        ...
        while time.monotonic() < deadline:
            status = server.latest_arm_status().get("robot_calibration", {})
